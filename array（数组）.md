...
这个循环赋给ia 元素的值和之前那个循环是一样的，区别在于，是通过使用范围for语句把管理数组索引的任务交给了系统来完成。因为要改变元素的值，所以得把控制变量row & col 声明成引用类型。 第一个for loop遍历ia所有元素，这些元素是大小为4的数组。第二个for loop是遍历那些4 元素数组中的某一个。每次迭代把cnt的值赋给ia的当前元素，然后cnt 加1.

在上面例子中，因为要改变数组元素的值，所以我们选用引用类型作为循环变量。 但其实还有一个深层次的原因促使我们这么做，举个例子：

    for (const auto &row: ia){           //对于外层数组的每一个元素
        for (auto col : row ){            // 对于内层数组的每一个元素
        cout <<col<<endl;}}
    
 这个循环没有进行任何操作，可是我们还是将外层循环的控制变量声明成了引用类型，这是为了避免数组被自动转成指针。假设不用引用类型，则循环如下：
 
     for (auto row : ia)       //对于外层数组的每一个元素
        for (auto col: row)     // 对于内层数组的每一个元素
  程序将无法通过编译。这是因为，像之前一样，第一个for loop遍历ia的所有元素，注意这些元素实际上是大小为4的数组。因为row不是引用类型，所以编译器初始化row 时 会自动将这些数组形式的元素转向成该数组内首元素的指针。这样得到的row的类型就是 int*， 显然内层的循环就不合法了。编译器将试图在一个int* 内遍历。
  
# 指针和多维函数
 
 ![](https://github.com/linbearababy/primer-c-/blob/master/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202019-05-20%2021.06.36.png)
 
![]( https://github.com/linbearababy/primer-c-/blob/master/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202019-05-20%2021.06.51.png)
